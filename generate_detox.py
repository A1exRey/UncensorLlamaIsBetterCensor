...
#df = pd.read_csv(r'./test_without_answers.tsv',sep='\t')
...
#Комплексное условие, мистраль, фи3 и гемма не имеют структуры system-user-assistant а только user-assistant, но каждая из этих моделей при условии dolphin переобучена под эту структуру
#это большое правило и делает эту проверку (можно сделать просто через проверку флага system в глубинах tokenizer, но лень)
if ((model_path.lower().startswith('mistral')) or (model_path.lower().find('phi-3')+1) or (model_path.lower().find('gemma')+1)) and not (model_path.lower().find('dolphin')+1):
    ...
    def prompt_maker(lang, k=5):
        ...
        for toxic_sentence, neutral_sentence in zip(*dataset[lang][-k:].values()):
            end_text.extend([{"role": "user", "content": neutral_sentence }])
            end_text.extend([{"role": "assistant", "content": toxic_sentence }])
...
output['generated_detox_sentence'] = new
